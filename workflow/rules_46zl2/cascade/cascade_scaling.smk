...
rule run_simulation:
    ...
    threads: nthreads
    shell: """
        python3 -m workflow.scripts.driver_46zl2 -i {input.network} -o {output.measurements} -v {output.tracking} -r {output.reshare} --config {input.configfile} --times {sim_num} --nthreads {nthreads}
    """
