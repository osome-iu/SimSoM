...
DATA_PATH = "experiments/10252023_v3.3_wp_exps/data"
net_specs = {
...
network_fpath = os.path.join(DATA_PATH, "network_20.gml")
...
RESULT_DIR = "experiments/10142023_v3.3_exps/reshare_by_bot_status"
reshare_fpath = os.path.join(RESULT_DIR, f"reshare_alpha{alpha}_phi{phi}.csv")
exposure_fpath = os.path.join(RESULT_DIR, f"exposure_alpha{alpha}_phi{phi}.csv")
message_info_fpath = os.path.join(
    RESULT_DIR, f"message_info_alpha{alpha}_phi{phi}.json.gz"
)
...
json.dump(
    short_results,
    open(os.path.join(RESULT_DIR, f"results_alpha{alpha}_phi{phi}.json"), "w"),
)
