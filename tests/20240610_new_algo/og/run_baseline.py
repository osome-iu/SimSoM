...
simulation_specs = json.load(
    open(
        os.path.join(DATA_PATH, "config", "baseline", "baseline.json"),
        "r",
    )
)
simulation_specs["epsilon"] = 0.001
simulation_specs["n_threads"] = 7
...
legal_specs = utils.remove_illegal_kwargs(simulation_specs, SimSomA1.__init__)
follower_sys = SimSomA1(network_fpath, **legal_specs)
