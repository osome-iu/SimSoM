...
RESULT_DIR = "/N/u/baotruon/BigRed200/simsom/tests/20240422_quality_timestep/results"
...
simulation_specs["converge_by"] = "both"
...
for run in range(no_runs):
    ...
    logger = utils.get_file_logger(
        log_dir="logs",
        full_log_path=os.path.join("logs", f"simulation_{run}__{now}.log"),
        also_print=True,
    )
    ...
    legal_specs["max_steps"] = random.choice(np.arange(200, 1000, 100))
