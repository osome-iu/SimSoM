...
rule run_simulation:
    ...
    shell: """
        python3 -m workflow.scripts.driver_46zl -i {input.network} -o {output.measurements} -v {output.tracking} --config {input.configfile} --times {sim_num} --nthreads {nthreads}
    """
