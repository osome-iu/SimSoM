...
class SimSom:
    def __init__(
        ...
        self.w_e = 0.5
        self.model_name = f"SimSomV4.3 (synchronous - permutate agent order); message 4.0; no recency; w_e={self.w_e}; no age reset"
    ...
    def _rank_newsfeed(self, newsfeed, w_e=0.5):
        ...
        # ranking = np.sum([w_e * engagement, w_p * popularity], axis=0) * recency
        # recency is not used but included for consistency to other versions
        recency = np.zeros(len(ages))
        ranking = np.sum([w_e * engagement, w_p * popularity], axis=0)
