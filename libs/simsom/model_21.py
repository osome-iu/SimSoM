...
class SimSom:
    def __init__(
        ...
        self.model_name = "SimSom 2.1 (all agents activated per step)"
    ...
    def simulation_step(self, agent):
        """
        ...
        Input:
        - agent (Graph vertex): agent doing the action
        """
        ...
        if len(feed) > 0 and random.random() > self.mu:
            ...
            meme = Message2(self.num_meme_unique, is_by_bot=agent["bot"], phi=self.phi)
    ...
    def _return_all_meme_info(self):
        for meme in self.all_memes:
            assert isinstance(meme, Message2)
