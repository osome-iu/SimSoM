...
ABS_PATH = '/N/project/simsom/simsom_v3/new_small/v4.1_10302023'
...
rule run_simulation:
    ...
    threads: nthreads
    shell: """
        python3 -m workflow.scripts.driver_41 -i {input.network} -o {output.measurements} -v {output.tracking} -r {output.reshare} --config {input.configfile} --times {sim_num} --nthreads {nthreads}
    """
