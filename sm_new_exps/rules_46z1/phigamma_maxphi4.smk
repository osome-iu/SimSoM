...
rule run_simulation:
    ...
    threads: nthreads
    shell: """
        python3 -m workflow.scripts.driver_46z1 -i {input.network} -o {output.measurements} -v {output.tracking} --config {input.configfile} --times {sim_num} --nthreads {nthreads}
    """
